        ...
        for i, space in zip(space_boxes, space_averages):
            ...
            if last_status[i[0]] != is_occupied:
                ...
                if last_ticks[i[0]] < 3:
                    ...
                    num = 0 if is_occupied else 1
                    occupancy = last_status

                    # send the data to the update function, the following data is sent:
                    # i[0]: the parkingspot ID
                    # num: the status code (0 for false and 1 for true)
                    # cameraArray[i[0] - 1]: the different camera ID's in cameraArray
                    senddata.send_update(i[0], num, cameraArray[i[0] - 1])
            else:
                last_ticks[i[0]] = 1
